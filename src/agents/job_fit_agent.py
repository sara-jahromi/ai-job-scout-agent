...
"""
        try:
            ...
            logger.error(f"Failed to generate or parse Gemini fit reasoning: {e}")
            return JobFitResult(
                fit_summary=f"Failed to generate LLM reasoning. Error: {str(e)}",
                strengths=["Unable to parse LLM response."],
                gaps=["Unable to parse LLM response."],
                apply_recommendation="Fallback recommendation: Please check your Gemini connection or prompt formatting."
            )
